...
import logging
import typing
from prenatalppkt.dto.efw_data import EfwEntry, FetusEfwData
...
class FetusEfwParser:
   ...
   def parse(self, json_data: typing.Dict[str, object]) -> typing.Optional[FetusEfwData]:
       """Parse the 'efws' list into FetusEfwData."""
       if not isinstance(json_data, dict):
           raise ValueError(f"Malformed argument: expected dict but got {type(json_data)}")
       ...
       logger.debug("Parsed %d EFW entries for fetus_number=%s", len(parsed_entries), fetus_number)
