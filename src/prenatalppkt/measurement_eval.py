...
from prenatalppkt.measurements.head_circumference_measurement import HeadCircumferenceMeasurement
...
class MeasurementEvaluation:
    ...
    # Mapping loader
    # ------------------------------------------------------------------ #
    def _load_mappings(self, path: Path) -> typing.Dict[BiometryType, SonographicMeasurement]:
        ...
        for meas_type, cfg in raw_mappings.items():
            ...
            for k, v in cfg.items():
                ...
                processed[prange] = TermBin(termid=v["id"], termlabel=v["label"], normal=v["normal"])
            if meas_type == BiometryType.HEAD_CIRCUMFERENCE:
                sonomsrt: SonographicMeasurement = HeadCircumferenceMeasurement(termbin_d=processed)
                measurement_d[meas_type] = sonomsrt
        return measurement_d
    

    def get_measurement_mapper(self, btype: BiometryType) -> SonographicMeasurement | None:
